...
precmd() {
  # command exit status
  # do this before any further commands are run
  EXIT_CODE=$?
  CMD_STATUS=󰋙
  STATUS_COLOR=green
  STATUS_CODE=''
  ...
  if [[ $(git rev-parse --is-inside-work-tree 2> /dev/null) == 'true' ]]; then
    ...
    STASHED=$( echo ${STASHED} | xargs) # trim mysterious whitespace
    (( STASHED > 0 )) && GIT_INFO="${GIT_INFO} *${STASHED}"
    ...
    STAGED=$(echo $GIT_STATUS | grep -c '^[MARD]')
  ...
  GAP_LENGTH=$(( COLUMNS-$#PWD-$#GIT_INFO-$#TIME-$#STATUS_CODE-38 ))
